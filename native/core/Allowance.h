...
namespace apmf::allowance {
    ...
    // Every engine address APMF hooks, calls or classifies with through its own
    // id is a row in native/VerifiedAddresses.h (generated by
    // tools/verified_addresses/ from OUR disassembly of 1.6.1170.0 and 1.5.97.0;
    // see Docs/VERIFIED-ADDRESSES.md). REL::SelfCheck::Run compares the Address
    // Library the game actually loaded with those rows once, the first time
    // anything asks. SeatVerified() is the one gate: true only for an address a
    // passing row covers; false logs the refusal by name and the caller must not
    // install / not call. On a build with no table (anything but the two above)
    // nothing is verified. Never substitutes the expected RVA, never installs anyway.
    ...
    int InstallOnVtables(std::span<const REL::VariantID> vtables, std::size_t slot,
        ...
        int n = 0;
        // mit-3.7 F1: the RTTI base the derivation walk compares against must itself
        // be a verified address, or the walk proves nothing -- refuse the whole list.
        if (expectedTypeDescriptor &&
            !SeatVerified(reinterpret_cast<std::uintptr_t>(expectedTypeDescriptor),
                          fmt::format("[{}] expected RTTI TypeDescriptor", tag))) {
            spdlog::error("[{}] NOTHING installed on this list (self-check refused its RTTI base).", tag);
            return 0;
        }
        for (const auto& id : vtables) {
            ...
            if (!SeatVerified(vt.address(), fmt::format("[{}] vtable slot 0x{:X}", tag, slot))) {
                continue;
            }
