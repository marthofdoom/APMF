...
| Shield | CheckShouldEquip (slot 0x0F) -- APMF TASK2 `EnableDualWieldPreference` | `0x817FC0` **CONFIRMED** | `0x77DC90` **CONFIRMED** | not derived (nothing placed on 1.7 this round) | **Added post-confirmation, reviewer re-derived** (Fable tier-3 on `feat/apmf-1.5.97-pass` c70767c, SEV-4; re-confirmed from the unpacked 1.5.97 image in the closing round). TASK2 rides inside APMF's Group C install loop and so opens on 1.5.97 with no row here until now. Both vtables (`0x18C9188` AE / `0x1681C28` SE, RTTI `.?AVCombatInventoryItemShield@@` on both) have **21 `.text` slots**; slot 0x0F on SE is `0x77DC90`, body `sub rsp,0x28; mov rcx,rdx; call 0x4FDE10; test al,al; sete al; add rsp,0x28; ret` -- byte-shape-identical to AE `0x817FC0` (`... call 0x559490 ...`; only the rel32 differs), so `ShieldEquip_t = bool(CombatInventoryItem*, CombatController*)` holds on SE. APMF installs this slot only after the Shield entry passes the slot-0x0C identity compare above; there is no independent 0x0F expected-value gate (see `AiCastSeats.cpp` ShieldEquipGateThunk's comment). |
