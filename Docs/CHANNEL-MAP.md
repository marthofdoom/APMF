...
| 6 | **Combat-target** ⭐ | `CombatGroup.targets[]` / `CombatController.targetHandle`, re-chosen each tick by threat | `StartCombat(target)` seeds; raw `targetHandle` write = override (fragile) | `StartCombat` | **steer DOCUMENTED; PIN GAP** |
