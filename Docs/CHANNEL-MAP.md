...
| 8+ACT | ↳ **Casting +ACT mode** (feat/cast-act, new) | Same claim (`RequestEx(kIntent_SelectSpell, {form=spell, ival=HAND, target=castTarget})`); `ival` (0 auto/1 right/2 left/3 dual) and `target` (2026-09-05 append, actor-only — `pos` reserved for a later location pass) now read | **APMF-EXECUTED, not arbitration-only** — a deliberate, scoped exception to the "client executes" rule above (MFO's own equivalent client-side execution is being REMOVED for this facet; `core/CastExecutor.cpp`) | APMF equips the resolved hand(s) (`ActorEquipManager::EquipSpell`, protected by feat/deny-perhand's per-hand ch.8b claim on that SAME hand), drives the OBSERVED animated sequence (`core/CastObserve.h`'s captured BeginCast→Charging→Charged→SpellFire→teardown), and falls back to `CastSpellImmediate` (guaranteed delivery) if the drive can't animate (VR / never selects / never charges). Target priority: the claim's own `param.target` (heal/buff-another — ch.6 `kIntent_CombatTarget` is the actor's FOE, not who to heal) → else a winning ch.6 claim (offense) → else self. A self-delivery spell resolved to a non-self target gets a delivery-flipped `kTargetActor` proxy (never AddSpell'd) so both placement and animation match | **Client declares (spell+hand+target); APMF owns execution end-to-end. `Release` restores the hand(s).** |
