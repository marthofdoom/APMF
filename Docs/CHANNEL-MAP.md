...
OPEN (18 channels + sub-splits: 1-17 plus ch.19; ch.18 is RESERVED for the unauthored attack-selection
spec, `Docs/SPEC-ATTACK-SELECTION-FACET.md`). Combat-target / combat-actions / casting (⭐) carry the weight.
...
| **19** | **Combat ENGAGE** (`kIntent_CombatEngage`, ABI v10, 2026-09-22) | Nothing in vanilla moves an actor to a foe it has not DETECTED. `StartCombat` on an undetected foe produces the engine's SEARCH behaviour, not an approach (MFO ENGINE_NOTES 0.12: a foe at 2792u never entered combat). The approach therefore has to be a real AI package | **COMPOSITE, not a new gate.** ch.19 installs NO seat of its own. One client claim expands into (a) an internal `kIntent_CombatTarget` claim (ch.6, arbitration) and (b) an internal `kIntent_OfferPackage` claim (ch.9) naming an **APMF-owned** Travel package from `Data/APMF.esl`, whose `Place to Travel` Location input `core/PackageData.cpp` points at the target ref (PLDT type 0, kNearReference). BOTH sub-claims are filed at the CLIENT's own basis, so a competing client still wins a facet it outbids for. Delivery is ch.9's already-proven 0x49 redirect + posted `EvaluatePackage` nudge | `channels/CombatEngage.cpp` claims the facet with `param.form` = the target actor, `fval` = arrival radius (0 => 128u), `ival` = `CombatEngageFlags`. The client does NOTHING else: no tick, no package to ship, no `StartCombat`, no `currentCombatTarget` write, no detection poke. `Repoint` switches the foe in place | **BUILT, OBSERVE-ONLY, not yet field-run.** v1 is the TRAVEL LEG ONLY (marth 2026-09-22, "for now lets try having it just travel to the enemy"): **no `StartCombat` and no 0xE4 `UpdateCombat` target PIN**, so `INVARIANTS #0` is UNTOUCHED by this channel. The approach ENDS on arrival / line of sight / detection / target gone, and the engine's own combat AI is expected to take the actor from there. Whether it does is precisely what the first field cycle measures -- and a NO is the evidence that would justify amending #0 for a bounded entry call, rather than assuming it. |
