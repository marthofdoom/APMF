...
| **8b** | **Cast EXECUTION** ⭐ **THE KEYSTONE** | **FIVE ENGINE SEATS** — the vfunc seats the combat AI's own cast decision is built out of: `0x0F CheckShouldEquip` (`core/EquipGate.cpp`, WHICH item enters the hands), `0x06 CheckStartCast` (WHETHER), `0x0A GetMagicTarget` (WHERE — handle form), `0x07 CheckStopCast` (HOW LONG), `0x0D SetupAimController` (AIM, `+0x30` target override) — the last four in `core/CastSeats.cpp`, on the **`CombatMagicCasterRestore` vtable ONLY** (0x0A's impl is the SHARED base: an unscoped redirect would aim Stagger/Disarm/Offensive effects at the ally). Plus the pre-existing exclusivity denies (`CastGate` 0x0A CheckCast, `EquipGate` 0x0F, both per-hand) and the bounded TTL auto-release. | **ARBITRATE + DENY + COMPOSE.** While the claim stands, the NPC's OWN AI selects, equips, charges, aims, fires and channels the claimed spell at the claimed target — engine animation, engine magicka, engine LOS/interrupts. `0x0F` is the ONE answer in this codebase that does NOT chain (the Restore templates' `CheckShouldEquip` IS the vanilla self/foe health gate `0x81f7c0`, which reads its target straight off the `CombatController` — there is no interposable seat, so chaining cannot express "heal the ally" and the other four seats are never called). See INVARIANTS #20. | **APMF FIRES NOTHING** — no `EquipSpell`, no `CastSpell`/`CastSpellImmediate`, no `StartCharge`/`InterruptCast`, no `NotifyAnimationGraph`, no `selectedSpells`/`desiredTarget` write. A **kSelf** spell still needs the delivery-flip proxy (`core/CastProxy.h`): `FindTargets`' Self branch (0x5bc98a) resolves to the caster's own ref and never reads `desiredTarget`. The proxy is minted + transiently taught on the WRITER thread at claim-apply and freed one main-thread hop AFTER the cleared claim publishes. | **BUILT — the AI itself performs the claimed cast. NOT yet field-run.** |
