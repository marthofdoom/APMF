...
| 6 | **Combat-target** ⭐ | `CombatGroup.targets[]` / `CombatController.targetHandle`, re-chosen each tick by threat | `StartCombat(target)` seeds; `Tick` re-asserts on `currentCombatTarget` drift = HOLD (known-incomplete #2); raw `targetHandle` write = override (fragile) | `StartCombat` | **HOLD via drift re-assert (#2 fill); true PIN-at-hook still GAP** |
