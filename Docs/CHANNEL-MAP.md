...
| 8+ACT | ↳ **Casting +ACT mode** (feat/cast-act, new, **OPT-IN**) | Same claim, `ival`'s `kActFlag_Drive` bit (value 4) SET: `RequestEx(kIntent_SelectSpell, {form=spell, ival=HAND\|kActFlag_Drive, target=castTarget})`. Bit CLEAR (incl. `ival==0`, the pre-+ACT shape) = row 8's base gate-only mode, UNCHANGED -- `core/CastExecutor.cpp` is never called (MFO's offense gambit stays on this path) | **APMF-EXECUTED, not arbitration-only, ONLY when opted in** — a deliberate, scoped exception to the "client executes" rule above (MFO's own equivalent client-side execution is being REMOVED for the facets that opt in; `core/CastExecutor.cpp`) | APMF equips the resolved hand(s) (`ActorEquipManager::EquipSpell`, protected by feat/deny-perhand's per-hand ch.8b claim on that SAME hand), drives the OBSERVED animated sequence (`core/CastObserve.h`'s captured BeginCast→Charging→Charged→SpellFire→teardown), and falls back to `CastSpellImmediate` (guaranteed delivery) if the drive can't animate (VR / never selects / never charges). Target priority: the claim's own `param.target` (heal/buff-another — ch.6 `kIntent_CombatTarget` is the actor's FOE, not who to heal) → else a winning ch.6 claim (offense) → else self. A self-delivery spell resolved to a non-self target gets a delivery-flipped `kTargetActor` proxy (TRANSIENT `AddSpell`/`RemoveSpell` around the drive window, 2026-09-05 field fix — `EquipSpell` can only select a spell the actor already knows) so both placement and animation match. Dropping the opt-in bit on a Repoint restores the hand exactly like Release. **AI-cast SUPPRESSION (feat/ai-cast-suppress):** for the WHOLE +ACT claim window the actor's own combat AI is barred from casting anything — `core/ActionGate.cpp` reads the +ACT bit off the winning ch.8 claim (same lock-free RCU snapshot the other gates use) and denies the magic CONTEXT-CREATION node + the four cast leaves as ForceFail's act()+pop() PAIR, so the AI never builds a magic context, never self-equips a spell, never fires alongside the drive. Melee/ranged/movement/targeting stay the AI's. The drive itself (EquipSpell/anim events/MagicCaster) never routes through the tree, so it is untouched | **Client declares (spell+hand+target) AND opts in; APMF owns execution end-to-end only then — and the AI's own magic branch is silent for the claim's life. `Release` (or dropping the bit) restores the hand(s) and re-arms the AI's magic branch.** |
