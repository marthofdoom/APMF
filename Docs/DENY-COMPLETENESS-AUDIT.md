...
| 22 | `kIntent_CombatReentryDeny` (ch.22, ABI v15, 2026-09-25) | SOURCE DENY of the engine's own combat ENTRY for a bounded window; INVARIANTS #0 (h) | **Every engine combat entry of the actor**: a `CombatController` is built only by CombatManager AE 46873 / 46874 (SE 45573 / 45574), called only from `Actor::StartCombat` (AE 38561 `0x6B6930` / SE 37608 `0x6251B0`) with the actor as `this` -- detection, being attacked, an ally's fight, a group join (r8), a script, another mod; ten direct callers per runtime, no data reference. The save-load rebuild (ctor caller AE 37650 / SE 36642) is out of scope: no claim crosses a load | Character vtable slot 0x99 (`IsDead`), chained, answered "dead" ONLY at StartCombat's own self-check return site (AE `0x6B69CA` / SE `0x625248`; verified call-site row `ReentryDeny.StartCombat.SelfIsDeadCall`), for the winning claim, inside the window, while the actor has NO controller, and not for ch.21's own entry (thread-local `ClientEntryScope`). The refusal precedes StartCombat's spinlock, re-arm equip and every other side effect | **YES for the declared facet (entry of an actor OUT of combat), not yet OBSERVED on the deck (principle 5).** It switches on NO substrate and writes nothing. **STATED HOLES, all deliberate:** (a) an actor ALREADY in combat is not stopped and its in-combat target-adds pass (StartCombat's in-combat path) -- the client ends the fight itself (`StopCombat`, once); (b) ch.21's own entry passes (#0 (h) condition 4), and the deny resumes when that fight ends; (c) other actors may keep fighting this actor (their `StartCombat` has them as `this`; hits, spells and pursuit are theirs) -- the client's declared consequence; (d) the save-load controller rebuild (claims never cross a load). **KNOWN MISS PATH, loud not masked:** a DLL that wraps slot 0x99 AFTER APMF with a non-tail call hides StartCombat's return address -- Engage warns once when the slot is not APMF's thunk, and `reentrydeny::Poll` logs `DENY MISSED` when a denied actor gains a controller under a live window without a ch.21 pass (with the seat's hit delta, so "never seen" is told apart). The seat also logs its first StartCombat self-check of the session (`seat OBSERVED`), claim or no claim |
