...
| 19 | `kIntent_CombatEngage` (ch.19, ABI v10, 2026-09-22) | COMPOSITE (ARBITRATE via ch.6 + DENY-redirect via ch.9) | the actor's own package choice (which ch.9's 0x49 seat already governs at the alias tier); the combat-target facet (which ch.6 already arbitrates) | NOTHING NEW. ch.19 adds no seat and no deny of its own: it files a ch.6 claim and a ch.9 claim at the client's basis and lets those two channels' existing behaviour stand (`channels/CombatEngage.cpp`) | **AS COMPLETE AS ch.6 AND ch.9 ARE, AND NO MORE -- deliberately.** v1 claims exactly two facets and denies nothing else. An actor under a ch.19 claim TRAVELS to the target and then behaves EXACTLY as vanilla would. That is the contract, not a shortfall: "walk there, then be normal" is what was asked for | **STATE THE HOLES PLAINLY -- every one of them is a deliberate v1 boundary, not an oversight.** (a) The approach leg inherits ch.9 row 9's gap verbatim: the 0x49 seat governs the ALIAS tier only, so a package selected through the non-alias procedure-tier path is not denied and can out-hold the approach. Observable as the `approach ABANDONED` safety-net line. (b) APMF does NOT enter combat and does NOT pin the engine's combat target -- no `StartCombat`, no 0xE4 `UpdateCombat` seat, `INVARIANTS #0` untouched. If the engine does not pick the fight up on arrival, ch.19 v1 simply delivered an actor next to a foe. (c) ch.6's own gap is unchanged (row 6): a COMPETING FRAMEWORK writing `currentCombatTarget` is not denied. (d) The claim takes NO attack selection (the ch.18 spec), NO casting (ch.8/8b), NO equip (ch.17), NO hands, NO aggression (ch.11) and NO movement block (ch.1 is all-or-nothing, so "fight but do not chase" is NOT on offer). (e) NO PERCEPTION IS FAKED: nothing writes a detection level or a combat-group knowledge entry. The actor really walks there under a real package and really sees the target, or the approach ends as ABANDONED and says so. (f) The approach does not set `kIgnoreCombat`, so a fight en route takes the actor -- by design (suppressing a combat reaction no client asked to suppress would itself be a deny hole). (g) Eight concurrent approaches; a ninth is REFUSED and logged, never served by sharing a package record (which would send two actors to one destination). |
