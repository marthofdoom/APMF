...
| 20 | `kIntent_TargetPin` (ch.20, ABI v13, 2026-09-25) | DENY (the engine's own target re-pick), at APMF's own seat; #0 (f) | (a) the ENGINE'S OWN re-pick of `currentCombatTarget` / `CombatController.targetHandle` inside `Actor::UpdateCombat` (target selector, threat); (b) a COMPETING FRAMEWORK's own write of the same fields; (c) the engine DROPPING the target (combat ends, foe lost / fled / undetected, StopCombat nulls it) | (a) Character vtable slot 0xE4 seat (`channels/TargetPin.cpp`): original first, then write the claimed target back when the engine holds a DIFFERENT one, for the actor's WINNING claim only; (b) nothing; (c) nothing, deliberately | **PARTIAL, and stated.** (a) is DENIED at one-update granularity: the write lands right after the engine's pick, inside the same per-actor job, so the NEXT update reads the claimed target -- but whatever the SAME update consumed from its own pick (its behaviour tree runs off the controller inside UpdateCombat) saw the engine's pick for that one update. That is a seat-time correction, not a source block (#2 label). (c) is NOT a hole: never writing when the engine holds no target is the rule that keeps combat ENTRY out (#0 (f) condition 2), and the claim claims the target facet only, never the existence of a fight | **STATE THE HOLES PLAINLY.** (1) **Source-level deny NOT BUILT:** the engine's own `CombatTargetSelector` choice is not suppressed at its source, so the one-update window in (a) stands. Closing it needs the selector seat found and verified on both images -- a separate brief. (2) **Competing framework (b) NOT DENIED -- row 6's gap, unchanged.** A hook chained OUTSIDE ours on the same slot (installed after us) writes last and wins; one chained inside us writes first and loses. Ordering between SKSE plugins at kDataLoaded is the plugin dispatch order, not ours. This includes MFO's own `Targeting` UpdateCombat hook during the transition: MFO must defer to the claim owner (MFO's task, not this repo's). (3) **Nothing else is claimed:** attack selection (ch.7 / the ch.18 design), casting (ch.8/8b), equip (ch.15/17), movement (ch.1), aggression (ch.11), perception and combat entry are all NOT touched -- one intent, one facet. (4) **The world's reaction is the client's** (CLAUDE.md principle 2 scope): crime, bounty, faction, aggression and allies joining are not denied and not undone on release. (5) **Threading:** the seat reads `a_this->combatController` once after the original returns on the same job thread; a StopCombat on ANOTHER thread against the same actor in that window (script, package evaluation, kill -- MFO ENGINE_NOTES 0.47 item 4) is the same race the engine's own update body has. Not closed here, not hidden. |
