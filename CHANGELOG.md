...
- `[PositionCast] bPositionCast=1` is new in `APMF.ini`. Set it to 0 and position casts are refused.
- New API revision (v11). It adds `FindEmptySpace` and `FindHostilesInSpace`, their structs, and the `kCastFlag_AtPosition` bit. Check `abiVersion >= 11` before calling either query or setting the bit, because an older Harbinger ignores the bit and makes an ordinary cast claim. A mod built against an older revision is unaffected.
